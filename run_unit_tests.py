...
try:
    ...
    unique = {norm(v) for v in paths.values() if v}

    if len(unique) == 1:
        print('  PASS: All systems use same path')
        passed += 1
    else:
...
try:
    ...
    if has_junk and has_12:
        print('  PASS: 12 classes with junk')
        passed += 1
    else:
...
try:
    ...
    if cfg.enable_phase3 and cfg.include_junk_class:
        print('  PASS: Phase3 and Junk enabled')
        passed += 1
    else:
...
try:
    from training_utils import CANONICAL_CLASSES_WITH_JUNK, STUDENT_NUM_CLASSES
    if STUDENT_NUM_CLASSES == 12 and len(CANONICAL_CLASSES_WITH_JUNK) == 12:
        print('  PASS: STUDENT_NUM_CLASSES=12')
        passed += 1
    else:
...
try:
    ...
    if r.status_code == 200:
        ...
        if 'junk' in class_names and len(class_names) == 12:
            print('  PASS: API returns 12 classes with junk')
            passed += 1
        else:
