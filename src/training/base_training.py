...
def prepare_datasets_for_backbone(backbone_name, size=224, test_split=0.2):
    ...
    try:
        ...
        NUM_CLASSES = len({s[1] for s in getattr(train_ds, 'samples', [])})
        ...
        if name == 'alexnet':
            ...
            if TIMM_AVAILABLE:
                try:
                    ...
                    logger.info("Created CSPResNet50 as DarkNet53 proxy")
                except:
                ...
                model = models.resnet50(pretrained=pretrained)
                model.fc = nn.Identity()
                logger.info("Using ResNet50 as DarkNet53 fallback (TIMM not available)")
...
def run_optimized_pipeline():
    ...
    logger.info("Enhanced epoch-wise logging: IMPLEMENTED")
...
if __name__ == "__main__":
    ...
    logger.info("\nENHANCED FEATURES:")
    logger.info("Spawn method for Windows compatibility")
    logger.info("Optimized worker initialization")
    logger.info("Persistent workers for performance")
    logger.info("Mixed precision training")
    logger.info("Memory-efficient data loading")
    logger.info("FIXED DarkNet53 feature dimension detection")
    logger.info(f"Ensemble training with {FUSION_TYPE} fusion")
    logger.info("Comprehensive visualizations")
    logger.info("ONNX and TorchScript export")
    logger.info("Deployment-ready model packages")
