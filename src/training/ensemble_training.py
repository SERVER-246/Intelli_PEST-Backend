            ...
            for o in out:
                if isinstance(o, torch.Tensor):
                    if o.ndim == 4:
                        ...
                        return o.flatten(1)
                    return o
        raise RuntimeError("FeatureExtractor couldn't process output")
    ...
    if backbone_name == 'alexnet':
        ...
        if TIMM_AVAILABLE:
            try:
                ...
                logger.info("Created CSPResNet50 for DarkNet53")
            except Exception as e:
...
def load_trained_model(model_id, deployment_dir, num_classes):
    ...
    try:
        ...
        with open(deployment_path / "class_mapping.json") as f:
            class_mapping = json.load(f)
        with open(deployment_path / "metadata.json") as f:
